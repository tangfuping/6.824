...
func (rf *Raft) electionLoop() {
	for !rf.killed() {
		...
		func() {
			...
			now := time.Now()
			timeout := ElectionTimeout + time.Duration(rand.Int31n(150))*time.Millisecond // 超时随机化
			...
			if rf.role == ROLE_CANDIDATES && elapses >= timeout {
				...
				DPrintf("RaftNode[%d] RequestVote starts, Term[%d] LastLogIndex[%d] LastLogTerm[%d]", rf.me, args.Term, args.LastLogIndex, args.LastLogTerm)
			...
			VOTE_END:
				rf.mu.Lock()
				defer func() {
					DPrintf("RaftNode[%d] RequestVote ends, finishCount[%d] voteCount[%d] Role[%s] maxTerm[%d] currentTerm[%d]", rf.me, finishCount, voteCount, rf.role, maxTerm, rf.currentTerm)
				}()
				// 如果角色改变了，则忽略本轮投票结果
				...
				if voteCount > len(rf.peers)/2 {
					rf.role = ROLE_LEADER
					rf.leaderId = rf.me
					rf.lastBroadcastTime = time.Unix(0, 0) // 立即执行appendEntries广播
					return
				}
			}
		}()
...
func (rf *Raft) appendEntriesLoop() {
	...
	for !rf.killed() {
		...
		func() {
			...
			// 100ms广播1次
			now := time.Now()
			if now.Sub(rf.lastBroadcastTime) < HeartBeatTimeout {
				return
			}
